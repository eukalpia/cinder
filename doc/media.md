...
`MediaPlayer` is the first-party media surface. It combines the direct video renderer, seek bar, playback buttons, volume, mute, speed selection, viewport modes, keyboard shortcuts, mouse controls, and runtime status. Press `Space` to play or pause, `F` to switch between complete-video and fullscreen-cover modes, or `C` to hide the controls:
