    ...
    return <Widget>[
      ...
      Positioned(
        ...
        child: _HudPanel(
          title: 'EVENTS',
          tone: _Tone.orange,
          lines: _events.take(4).map((event) => '› $event').toList(growable: false),
        ),
      ),
      Positioned(
        right: 2,
...
class _SceneBuffer {
  ...
  void line(int x1, int y1, int x2, int y2, _Tone tone, int tick) {
    ...
    while (true) {
      ...
      put(x, y, pulse ? '◆' : (dx >= dy ? '─' : '│'), pulse ? _Tone.orange : tone, 2);
      if (x == x2 && y == y2) break;
      final e2 = error * 2;
      if (e2 > -dy) {
        error -= dy;
    ...
    for (var y = 0; y < height; y++) {
      ...
      for (var x = 0; x < width; x++) {
        ...
        if (tone != currentTone && run.isNotEmpty) {
          spans.add(TextSpan(text: run.toString(), style: _styleFor(currentTone)));
          run.clear();
          currentTone = tone;
        }
...
List<_Building> _layoutBuildings(
  ...
  final specs = <_Building>[
    _Building(name: 'Cinder Core', x: centerX - sx(12), y: centerY - sy(4), width: sx(20), height: sy(5), depth: sx(4), tone: _Tone.purple, seed: 1),
    _Building(name: 'Widget Tower', x: centerX - sx(47), y: centerY - sy(15), width: sx(12), height: sy(12), depth: sx(3), tone: _Tone.purple, seed: 2),
    _Building(name: 'Element Stack', x: centerX - sx(28), y: centerY - sy(18), width: sx(10), height: sy(15), depth: sx(3), tone: _Tone.cyan, seed: 3),
    _Building(name: 'Render Spire', x: centerX - sx(8), y: centerY - sy(20), width: sx(11), height: sy(17), depth: sx(3), tone: _Tone.purple, seed: 4),
    _Building(name: 'Buffer Array', x: centerX + sx(17), y: centerY - sy(18), width: sx(12), height: sy(15), depth: sx(3), tone: _Tone.orange, seed: 5),
    _Building(name: 'Diff Tower', x: centerX + sx(39), y: centerY - sy(15), width: sx(11), height: sy(12), depth: sx(3), tone: _Tone.purple, seed: 6),
    _Building(name: 'Input Hub', x: centerX - sx(56), y: centerY + sy(4), width: sx(12), height: sy(8), depth: sx(3), tone: _Tone.orange, seed: 7),
    _Building(name: 'Focus Grid', x: centerX - sx(35), y: centerY + sy(8), width: sx(12), height: sy(9), depth: sx(3), tone: _Tone.purple, seed: 8),
    _Building(name: 'Scheduler', x: centerX + sx(22), y: centerY + sy(8), width: sx(12), height: sy(9), depth: sx(3), tone: _Tone.orange, seed: 9),
    _Building(name: 'Web Backend', x: centerX + sx(45), y: centerY + sy(4), width: sx(12), height: sy(8), depth: sx(3), tone: _Tone.purple, seed: 10),
    _Building(name: 'Image Node', x: centerX - sx(18), y: centerY + sy(14), width: sx(10), height: sy(7), depth: sx(3), tone: _Tone.pink, seed: 11),
    _Building(name: 'Terminal Link', x: centerX + sx(4), y: centerY + sy(15), width: sx(10), height: sy(7), depth: sx(3), tone: _Tone.cyan, seed: 12),
  ];
  ...
  for (var y = 0; y < buffer.height; y++) {
    for (var x = 0; x < buffer.width; x++) {
      ...
      if (hash % 47 == 0) {
        ...
        buffer.put(x, y, bright ? '✦' : '·', bright ? _Tone.pink : _Tone.dim, 1);
      } else if (hash % 79 == 0) {
...
void _drawNetwork(
  ...
  for (var index = 0; index < hubs.length; index++) {
    ...
    buffer.line(centerX, centerY + 2, hub.$1, hub.$2, _Tone.purple, tick + index * 3);
    if (index > 0) {
      final previous = hubs[index - 1];
      buffer.line(previous.$1, previous.$2, hub.$1, hub.$2, _Tone.orange, tick + index * 5);
    }
  }
...
void _drawBuilding(
  ...
  if (highlighted && width >= 8) {
    ...
    buffer.text(x + (width - clipped.length) ~/ 2, y + depth, clipped, _Tone.white, 7);
  }
}
...
void _drawCore(
  ...
  buffer.text(x - 2, y + 4, '│${_center('>_  CINDER', width)}│', _Tone.white, 10);
  ...
  for (var row = 0; row < flameHeight; row++) {
    ...
    final half = math.max(0, (ratio * (compact ? 5 : 8)).round() + (phase == 0 ? 1 : 0));
    ...
    for (var dx = -half; dx <= half; dx++) {
      ...
      final tone = inner
          ...
          : row < flameHeight ~/ 3
              ? _Tone.pink
              : _Tone.purple;
      buffer.put(centerX + dx + ((row + tick) % 3) - 1, flameY, glyph, tone, 12);
    }
  }
}
