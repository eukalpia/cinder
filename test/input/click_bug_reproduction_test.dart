...
void main() {
  group('Click Bug Reproduction', () {
    test('click with isMotion=false should work (baseline)', () async {
      await testCinder('baseline click', (tester) async {
        int tapCount = 0;
    ...
    test('click with isMotion=true should now work (bug fixed)', () async {
      await testCinder('isMotion bug fixed', (tester) async {
        int tapCount = 0;
        ...
        expect(
          tapCount,
          1,
          reason: 'Fix: clicks with isMotion=true now work via state tracking',
        );
      }, debugPrintAfterPump: true);
    });

    test('mixed motion and click events', () async {
      await testCinder('mixed events', (tester) async {
        int tapCount = 0;
    ...
    test('real world scenario: hover then click', () async {
      await testCinder('hover then click', (tester) async {
        int tapCount = 0;
