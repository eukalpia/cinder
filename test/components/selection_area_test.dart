...
void main() {
  group('SelectionArea', () {
    test(
      'selection completion inserts newline when moving to a new row',
      () async {
        await testCinder('selection completion', (tester) async {
          String? lastChanged;
          String? completed;
    ...
    test('clears selection when text content changes', () async {
      await testCinder('clears selection on text change', (tester) async {
        String? lastChanged;
    ...
    test(
      'starts selection when pressing on whitespace and dragging into text',
      () async {
        await testCinder('drag from whitespace', (tester) async {
          String? lastChanged;
    ...
    test('selection crosses a non-selectable gap between widgets', () async {
      await testCinder('cross boundary selection', (tester) async {
        String? completed;
    ...
    test('selection updates on mouse release position', () async {
      await testCinder('selection release update', (tester) async {
        String? completed;
    ...
    test('drag below last line clamps selection to end', () async {
      await testCinder('drag below last line', (tester) async {
        String? completed;
    ...
    test('reanchors when anchored selectable is replaced', () async {
      await testCinder('reanchor on replace', (tester) async {
        String? lastChanged;
    ...
    test('selectionColor setter updates cached selectables', () async {
      await testCinder('selectionColor setter', (tester) async {
    ...
    test('onEnter clears left button pressed state', () async {
      await testCinder('onEnter clears left button pressed', (tester) async {
        String? lastChanged;
    ...
    test('onHover with left button down triggers selection start', () async {
      await testCinder('hover left button down', (tester) async {
        String? lastChanged;
    ...
    test('onExit finalizes selection when leaving region', () async {
      await testCinder('exit finalizes selection', (tester) async {
        String? completed;
    ...
    test('anchor context removed clears selection', () async {
      await testCinder('anchor context removed', (tester) async {
        String? lastChanged;
    ...
    test('onExit handles left button release when invoked directly', () async {
      await testCinder('onExit direct', (tester) async {
        RenderSelectionArea? renderObject;
        String? completed;
        ...
        annotation.onEnter?.call(
          const MouseEvent(button: MouseButton.left, x: 1, y: 0, pressed: true),
        );
        annotation.onExit?.call(
          const MouseEvent(
            button: MouseButton.left,
            x: 20,
            y: 5,
    ...
    test('dispose cleans drag state when unmounted mid-drag', () async {
      await testCinder('dispose mid-drag cleanup', (tester) async {
        await tester.pumpWidget(
          Container(
            width: 10,
            height: 2,
    ...
    test(
      'ListView items with multiple selectables use positional sort',
      () async {
        await testCinder('listview positional sort', (tester) async {
          String? completed;
              ...
              child: SelectionArea(
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) =>
                      Row(children: [Text('A$index'), Text('B$index')]),
                ),
              ),
            ),
          );
    ...
    test('ListView returns to lazy mode after selection drag ends', () async {
      await testCinder('listview lazy restore', (tester) async {
        await tester.pumpWidget(
          Container(
            width: 20,
            height: 3,
    ...
    test('nearest selectable uses left-side distance', () async {
      await testCinder('nearest selectable left', (tester) async {
        String? lastChanged;
    ...
    test('multiline selection inserts newlines', () async {
      await testCinder('multiline newlines', (tester) async {
        String? completed;
    ...
    test('multiline selection inserts spaces for same-row widgets', () async {
      await testCinder('multiline spaces', (tester) async {
        RenderSelectionArea? renderObject;
    ...
    test('excludes offscreen text from selection output', () async {
      await testCinder('offscreen exclusion', (tester) async {
        String? completed;
        ...
        await tester.pumpWidget(
          Container(
            ...
            child: SelectionArea(
              ...
              child: Stack(
                children: [
                  Positioned(left: 0, top: 0, child: const Text('Visible')),
                  Positioned(left: 0, top: -1, child: const Text('Hidden')),
                ],
              ),
            ),
          ),
    ...
    test('backward selection across three widgets', () async {
      await testCinder('backward selection', (tester) async {
        String? completed;
        ...
        await tester.pumpWidget(
          Container(
            ...
            child: SelectionArea(
              ...
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: const [Text('Alpha'), Text('Bravo'), Text('Charlie')],
              ),
            ),
          ),
        );
    ...
    test(
      'horizontal flex does not expand selection to sibling row items',
      () async {
        await testCinder('row group expansion', (tester) async {
          String? completed;
    ...
    test('selectionColor same-value early return and actual update', () async {
      await testCinder('selection color update', (tester) async {
        String? lastChanged;
    ...
    test('null onSelectionCompleted callback', () async {
      await testCinder('null completed callback', (tester) async {
        String? lastChanged;
    ...
    test('anchor below and focus above selectable', () async {
      await testCinder('anchor below focus above', (tester) async {
        String? completed;
    ...
    test('anchor above and focus below selectable', () async {
      await testCinder('anchor above focus below', (tester) async {
        String? completed;
    ...
    test('empty selectables during pointer move', () async {
      await testCinder('empty selectables', (tester) async {
        String? lastChanged;
    ...
    test('wheel scroll during active drag', () async {
      await testCinder('wheel during drag', (tester) async {
        String? lastChanged;
    ...
    test('SingleChildScrollView viewport clipping', () async {
      await testCinder('viewport clipping', (tester) async {
        String? completed;
    ...
    test('ListView context selection', () async {
      await testCinder('listview selection', (tester) async {
        String? completed;
    ...
    test('multiline wrapped text selection', () async {
      await testCinder('multiline wrapped text', (tester) async {
        String? completed;
    ...
    test('mouse exit during drag triggers selection completed', () async {
      await testCinder('mouse exit during drag', (tester) async {
        String? completed;
    ...
    test('onHover button state transitions', () async {
      await testCinder('hover button transitions', (tester) async {
        String? lastChanged;
        String? completed;
    ...
    test('context entries become empty after anchor set', () async {
      await testCinder('empty context after anchor', (tester) async {
        String? lastChanged;
    ...
    test('drag to the bottom edge auto-scrolls the viewport; mid-viewport '
        'motion does not', () async {
      await testCinder('edge auto-scroll bottom', (tester) async {
    ...
    test('edge auto-scroll stops once the drag is released', () async {
      await testCinder('edge auto-scroll stops on release', (tester) async {
        final controller = ScrollController();
  ...
  Widget build(BuildContext context) {
    return SelectionArea(
      ...
      child: _useListView
          ...
          : Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: const [Text('New1'), Text('New2')],
            ),
    );
  }
}
...
class _SelectionAreaUnmountHarnessState
  ...
  Widget build(BuildContext context) {
    ...
    return const SelectionArea(child: Text('Hello'));
  }
}
