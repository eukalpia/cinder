...
void main() {
  group('Builder', () {
    test('builds widget from builder callback', () async {
      await testCinder('basic builder', (tester) async {
        await tester.pumpWidget(
          Container(
            width: 30,
            height: 5,
            child: Builder(builder: (context) => Text('Built by Builder')),
          ),
        );
    ...
    test('provides BuildContext to builder callback', () async {
      await testCinder('builder context', (tester) async {
    ...
    test('can access inherited widgets through context', () async {
      await testCinder('builder inherited access', (tester) async {
        await tester.pumpWidget(
          Container(
            width: 40,
            height: 5,
    ...
    test('rebuilds when parent rebuilds', () async {
      await testCinder('builder rebuilds', (tester) async {
        var buildCount = 0;
    ...
    test('works nested inside other components', () async {
      await testCinder('nested builder', (tester) async {
        await tester.pumpWidget(
          Container(
            width: 40,
            height: 10,
            child: Column(
              children: [
                Text('Header'),
                Builder(builder: (context) => Text('Built content')),
                Text('Footer'),
              ],
            ),
          ),
    ...
    test('can return complex widget trees', () async {
      await testCinder('complex builder output', (tester) async {
        await tester.pumpWidget(
          Container(
            width: 40,
            height: 10,
    ...
    test('visual development - builder in layout', () async {
      await testCinder('visual builder demo', (tester) async {
        await tester.pumpWidget(
          Container(
            width: 50,
            height: 10,
