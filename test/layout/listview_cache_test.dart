...
void main() {
  group('ListView cache optimization', () {
    test(
      'itemBuilder is only called once per unique index during layout',
      () async {
        await testCinder('cache optimization test', (tester) async {
          ...
          expect(
            initialBuildCount,
            equals(initialUniqueCount),
            reason: 'Each item should only be built once during initial layout',
          );
    ...
    test('non-lazy mode also benefits from cache optimization', () async {
      await testCinder('non-lazy cache test', (tester) async {
        final scrollController = ScrollController();
        int buildCount = 0;
  ...
  group('ListView rebuild on state change', () {
    test('items rebuild when parent state changes (selection)', () async {
      await testCinder('selection rebuild test', (tester) async {
          ...
          return Container(
            ...
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                ...
                return Text('${isSelected ? ">" : " "} Item $index');
              },
            ),
          );
        }
        ...
        expect(
          tester.terminalState.containsText('> Item 1'),
          isFalse,
        ...
        expect(
          tester.terminalState.containsText('> Item 2'),
          isTrue,
    ...
    test('items rebuild when itemCount changes', () async {
      await testCinder('item count change test', (tester) async {
        final scrollController = ScrollController();
        ...
        expect(
          tester.terminalState.containsText('Item 5'),
          isFalse,
    ...
    test('cache is cleared when widget updates', () async {
      await testCinder('cache clear on update test', (tester) async {
        int buildCount = 0;
        ...
        expect(
          tester.terminalState.containsText('Version 2'),
          isTrue,
          reason: 'Items should show updated content after rebuild',
        );
        expect(
          tester.terminalState.containsText('Version 1'),
          isFalse,
  ...
  group('ListView performance with large lists', () {
    test('lazy mode handles large item counts efficiently', () async {
      await testCinder('large list performance test', (tester) async {
        final scrollController = ScrollController();
        int buildCount = 0;
        ...
        expect(
          buildCount,
          lessThan(20),
          reason: 'Lazy mode should only build visible items, not all 10,000',
        );
    ...
    test('scrolling through list only builds new items', () async {
      await testCinder('scroll efficiency test', (tester) async {
        final scrollController = ScrollController();
        final builtIndices = <int>{};
