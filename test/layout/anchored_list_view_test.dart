...
void main() {
  test('ExtentIndex keeps stable-ID measurements across reorder and eviction', () {
  ...
  test('100k variable-height history starts and jumps without eager building',
      () async {
    ...
    await testCinder('large anchored history', (tester) async {
      await tester.pumpWidget(_HistoryHost(
        key: key,
  ...
  test('late media relayout corrects scroll without moving the anchor', () async {
    final controller = AnchoredListController();
    final key = GlobalKey<_HistoryHostState>();
  ...
  test('controller saves and restores an independent position per chat',
      () async {
    final controller = AnchoredListController();
    final key = GlobalKey<_HistoryHostState>();
  ...
  test('legacy lazy ListView retains an O(log n) variable-extent start index',
      () async {
