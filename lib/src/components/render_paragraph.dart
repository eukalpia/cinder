  ...
  List<List<StyledTextSegment>> _mapSegmentsToLines(
    ...
    for (var lineIndex = 0; lineIndex < lines.length; lineIndex++) {
      ...
      for (var i = 0; i < sourceCount; i++) {
        lastStyle = charIndex < charStyles.length
            ? charStyles[charIndex++].$2
            : lastStyle;
        append(graphemes[i], lastStyle);
      }
      if (ellipsized) {
        if (sourceCount == 0 && charIndex < charStyles.length) {
          lastStyle = charStyles[charIndex].$2;
        }
      ...
      if (charIndex < charStyles.length && charStyles[charIndex].$1 == '\n') {
        charIndex++;
      }
      flushSegment();
