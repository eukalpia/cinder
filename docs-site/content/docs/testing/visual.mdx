...
Visual testing lets you see what your widget looks like during tests. This helps debug layout and rendering issues.
...
When enabled, the terminal state prints after each `pumpWidget()` call.

## Development workflow

Use visual testing during development:

### Step 1: Create widget with visual test

```dart
test('new widget visual', () async {
  await testCinder(
    'see layout',
    (tester) async {
...
test('develop new feature', () async {
  await testCinder(
    'feature visual',
    (tester) async {
      await tester.pumpWidget(NewFeature());
...
This shows exactly what your widget renders.
...
- [Test Matchers](/docs/testing/matchers) - Learn all matchers
- [Writing Tests](/docs/testing/basics) - Test basics
- [Widgets](/docs/fundamentals/components) - Build components
