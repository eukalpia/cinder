...
```dart
expect(tester.terminalState, matchesSnapshot('my-widget'));
```
...
test('displays welcome message', () async {
  await testCinder('welcome', (tester) async {
    await tester.pumpWidget(WelcomeScreen());

    expect(tester.terminalState, containsText('Welcome to Cinder'));
...
test('shows error in red', () async {
  await testCinder('error style', (tester) async {
    await tester.pumpWidget(ErrorMessage(message: 'Failed'));
...
test('empty list shows placeholder', () async {
  await testCinder('empty', (tester) async {
    await tester.pumpWidget(ItemList(items: []));
...
test('title in correct position', () async {
  await testCinder('layout', (tester) async {
...
```dart
test('widget matches snapshot', () async {
  await testCinder('snapshot', (tester) async {
    await tester.pumpWidget(ComplexWidget());

    expect(tester.terminalState, matchesSnapshot('complex-widget'));
  });
});
```
