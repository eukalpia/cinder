...
The debug overlay is a built-in developer tool that shows real-time performance metrics and helps you identify rendering issues. It's automatically available in every Cinder app—no setup required.
...
- **Build** - Time spent building widget trees
...
When debug mode is active, Cinder enables the "repaint rainbow" effect. Components that repaint are briefly colored, showing you exactly which parts of your UI are updating.

This is invaluable for debugging unnecessary repaints:
- If a widget keeps changing color when it shouldn't, something is triggering unneeded rebuilds
- If everything flashes when you expect only one widget to update, state might be too high in the tree

## Extended logging with cinder logs
...
Keep `cinder logs` open in a second terminal while developing to monitor your app's behavior.
...
## Example: Debug-aware widget

Here's a complete example showing a widget that reacts to debug mode:

```dart
import 'package:cinder/cinder.dart';

class DebugStatus extends StatefulWidget {
