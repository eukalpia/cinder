...
description: Managing state in your Cinder apps
icon: database
---

State is data that changes over time. This guide shows you how to manage state in Cinder apps using `setState()` and other patterns.

## Using setState

The `setState()` method tells Cinder to rebuild your widget with new state.
...
2. Cinder marks the widget as needing rebuild
3. The `build()` method runs with the new state
4. The UI updates

### What setState does

`setState()` schedules a rebuild. It doesn't rebuild immediately—Cinder batches updates for efficiency.
...
Always check `mounted` before calling `setState()` in async callbacks. The widget might be removed from the tree before the operation completes.
...
- Widgets need to communicate with each other
...
If you need to respond to changes, use lifecycle methods like `didUpdateWidget()` or `didChangeDependencies()`.

## Advanced: Riverpod

For complex state management, Cinder supports Riverpod. Riverpod provides:
...
See the Riverpod documentation for details on using providers with Cinder.

## Next steps

- [Hot Reload](/docs/fundamentals/hot-reload) - Fast development with hot reload
- [Widgets](/docs/fundamentals/components) - Review widget types
- [Building UIs](/docs/fundamentals/building-uis) - Compose components
