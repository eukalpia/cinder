...
Cinder includes a theming system that makes it easy to apply consistent colors across your app. You can use built-in themes, customize them, or create your own.
...
Access theme colors in any widget:
...
`CinderApp` can automatically detect your terminal's brightness and apply an appropriate theme:
...
When no `theme` is provided, cinder will:
...
Cinder includes six popular themes:
...
Widgets that call `TuiTheme.of(context)` automatically rebuild when the theme changes. This happens because `TuiTheme` is an `InheritedWidget` that notifies dependents.
...
await testCinder('renders in dark theme', (tester) async {
  await tester.pumpWidget(
    TuiTheme(data: TuiThemeData.dark, child: MyWidget()),
  );
  // Verify appearance
});

await testCinder('renders in light theme', (tester) async {
  await tester.pumpWidget(
    TuiTheme(data: TuiThemeData.light, child: MyWidget()),
  );
  // Verify appearance
});
```
