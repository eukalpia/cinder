---
title: Navigation
description: Managing screens and navigation in your Cinder apps
icon: compass
---

Navigation in Cinder uses a stack-based system. You push routes onto the stack to navigate forward and pop them to go back. This works similarly to Flutter's navigation.

## Quick start

Use the `Navigator` widget to manage screens in your app:
...
- Supports named routes and direct widget pushing
...
|----------|------|-------------|
| `home` | `Widget?` | Initial screen to display |
| `routes` | `Map<String, Widget Function(BuildContext)>?` | Named routes map |
...
Push a widget directly (creates a route for you):
...
// Or with a widget
Navigator.of(context).pushReplacementWidget(const NewPage());
```
...
|-----------|------|---------|-------------|
| `builder` | `Widget Function(BuildContext)` | required | Dialog content |
...
Cinder includes a built-in logging observer for debugging:
...
// Less clear
Navigator.of(context).pushWidget(const SettingsPage());
```
...
- [Widgets](/docs/fundamentals/components) - Build reusable components
