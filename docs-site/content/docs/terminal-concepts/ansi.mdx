...
ANSI escape codes are special character sequences that control how terminals display text. This guide explains what they are and how Cinder uses them behind the scenes.
...
Cinder provides color constants like `Colors.red`, `Colors.blue`, etc. You use them like this:

```dart
Text('Hello', style: TextStyle(color: Colors.red))
```

Cinder handles the ANSI codes for you. You never write `\x1b[31m` directly.
...
In Cinder, you apply styles through `TextStyle`:
...
Cinder handles all cursor positioning. When you use `Column`, `Row`, or other layouts, Cinder calculates where each widget goes and moves the cursor automatically.
...
Cinder uses these to optimize rendering. When your UI changes, Cinder updates only the parts that changed instead of redrawing everything.
...
With Cinder, you write declarative UI code:
...
Cinder generates the correct ANSI codes and handles terminal differences for you.
