...
Cinder provides predefined terminal colors:
...
`AsciiText` renders text as large ASCII art using customizable fonts:
...
Four built-in fonts are available:

```dart
// Standard (default) - 5 lines high, block style
AsciiText('HELLO', font: AsciiFont.standard)

// Banner - 7 lines high, large characters
AsciiText('HELLO', font: AsciiFont.banner)

// Block - 6 lines high, bold appearance
AsciiText('HELLO', font: AsciiFont.block)

// Slim - 5 lines high, minimalist style
AsciiText('HELLO', font: AsciiFont.slim)
```

### Styling

Apply styles like regular text:
...
Built-in fonts support:
- Letters A-Z (case-insensitive)
- Numbers 0-9
- Common punctuation: `!?.,;:-_()[]{}/<>+=*#@&%$^~`\`'"`
- Space

Unsupported characters display as fallbacks.

### Custom fonts

Create custom fonts by extending `AsciiFont`:
...
class MyFont extends AsciiFont {
  ...
  @override
  int get letterSpacing => 1;  // Optional, defaults to 1
...
AsciiText('AB', font: MyFont())
```

### Best practices

- Use ASCII text for headers, logos, and splash screens
- Keep text short—ASCII art is wide
- Test at your target terminal width (fonts can be 6-10 chars per letter)
- Prefer `standard` or `slim` for space-constrained layouts

## Next steps

- [Input](/docs/components/input) - Add user input
- [Layout](/docs/components/layout) - Arrange text with layout components
- [Scrolling](/docs/components/scrolling) - Display long text with scrolling
