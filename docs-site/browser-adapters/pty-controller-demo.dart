...
class _PtySandboxAppState extends State<PtySandboxApp> {
  ...
  void _submit(String input) {
    ...
    setState(() {
      _lines.add(r'sandbox@cinder:~$ ' + command);
  ...
  List<String> _execute(String command) {
    ...
    if (command == 'ls') {
      return const ['README.md  demo  capabilities.json'];
    }
    if (command == 'cat demo') {
      return const [
    ...
    return ['command not available in sandbox: $command'];
  }
      ...
      child: Column(
        children: [
          ...
          Container(
            ...
            child: Row(
              children: [
                const Text('sandbox@cinder:~$ ', style: TextStyle(color: Colors.green)),
                Expanded(
                  child: TextField(
